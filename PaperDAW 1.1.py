...
class Track(QWidget):
    def __init__(self, name):
        ...
        self.client = anthropic.Client(api_key="Your API Key Here")  # Replace with your actual API key
...
Now go ahead and start! your job is to create your own ideas for 4 or 8 or 16 or 32 or 64 bar loops for each of the tracks. I suggest this order to be most fruitful: Metronome first (decide if you want to be in 4/4 or something more unique), then hi hat, then Kick/snare, then bass, then melody, and lastly Lyrics. If you have trouble figuring out how to make them sync up or sound good, you can always look at my example. Good luck and have fun!\n\nAssistant:"""
        ...
        # List of words that will trigger the cutoff
        cutoff_words = ['Hi-Hat', r'\(Repeat']
        
        # Check if the response contains the expected notation format
        if '. .' not in generated_text:
            self.text_input.setText("Croooaak!\n@ . . . $ . . . $ . . . $ . . . @ . . . $ . . . $ . . . $ . . .")
            return

        # Use regular expressions to process the text
        pattern = r'(?:.*?:){1,2}(.*?)(?:(?:' + '|'.join(cutoff_words) + ').*)?$'
        match = re.search(pattern, generated_text, re.DOTALL | re.IGNORECASE)
        
        if match:
            processed_text = match.group(1).strip()
            
            # Check if there's a second colon after the captured content
            second_colon_match = re.search(r'(.*?:.*?:)(.*)', processed_text, re.DOTALL)
            if second_colon_match:
                processed_text = second_colon_match.group(2).strip()
        else:
            processed_text = "Croooaak!\n@ . . . $ . . . $ . . . $ . . . @ . . . $ . . . $ . . . $ . . ."

        # Final check to ensure the processed text contains the expected notation
        if '. .' not in processed_text:
            processed_text = "Croooaak!\n@ . . . $ . . . $ . . . $ . . . @ . . . $ . . . $ . . . $ . . ." 

        self.text_input.setText(processed_text)
...
Now go ahead and start! your job is to create your own ideas for 4 or 8 or 16 or 32 or 64 bar loops for each of the tracks. I suggest this order to be most fruitful: Lyrics first, then melody, then metronome, then hi hat, then bass, and lastly kick/snare. If you have trouble figuring out how to make them sync up or sound good, you can always look at my example. Good luck and have fun!\n\nAssistant:"""
        ...
        error_message = f"I'm not feeling very lyrical at the moment{random_periods()} Try again later{random_periods()}"

        # Lists of start and end markers
        start_markers = ['Lyrics:', '):']
        end_markers = ['Melody', ' Now']

        # Check if the response contains at least one '-'
        if '-' not in generated_text:
            self.text_input.setText(error_message)
            return

        # Create the regex pattern from the lists
        start_pattern = '|'.join(map(re.escape, start_markers))
        end_pattern = '|'.join(map(re.escape, end_markers))
        pattern = f'(?:{start_pattern})(.*?)(?:{end_pattern}|$)'

        # Use regular expressions to find and extract the lyrics
        match = re.search(pattern, generated_text, re.DOTALL | re.IGNORECASE)
        
        if match:
            processed_text = match.group(1).strip()
            
            # Check if the processed text contains at least one '-'
            if '-' not in processed_text:
...
Now go ahead and start! your job is to create your own ideas for 4 or 8 or 16 or 32 or 64 bar loops for each of the tracks. I suggest this order to be most fruitful: Kick/Snare first, then hi hat, then Lyrics, then bass, then melody, and lastly metronome. If you have trouble figuring out how to make them sync up or sound good, you can always look at my example. Good luck and have fun!\n\nAssistant:"""
        ...
        error_message = f"{random_periods()}*Glares at you*{random_periods()}"""

        # Lists of start and end markers
        start_markers = ['Track:', '):', 'BassKick:']
        end_markers = [' Now', 'This', "I've", 'Hi-Hat']
...
Now go ahead and start! your job is to create your own ideas for 4 or 8 or 16 or 32 or 64 bar loops for each of the tracks. I suggest this order to be most fruitful: Hi-Hat first, then Kick/Snare, then Bass, then Lyrics, then melody, and lastly metronome. If you have trouble figuring out how to make them sync up or sound good, you can always look at my example. Good luck and have fun!\n\nAssistant:"""
        ...
        error_message = f"""Here's a detailed ASCII art representation of a top hat:\n\n{random_periods()}     
    ##########\n
    ##########\n
    ##########\n
    ##########\n
...
{random_periods()}"""
        ...
        # Lists of start and end markers
        start_markers = ['Track:', '):']
        end_markers = ['Kick/', 'Now', 'This', "I've", 'Kick /']
...
Now go ahead and start! your job is to create your own ideas for 4 or 8 or 16 or 32 or 64 bar loops for each of the tracks. I suggest this order to be most fruitful: Melody first, then Lyrics, then Bass, then Hi-Hat, then Kick/Snare, and lastly metronome. If you have trouble figuring out how to make them sync up or sound good, you can always look at my example. Good luck and have fun!\n\nAssistant:"""
        ...
        # List of words that will trigger the cutoff
        cutoff_words = ['Lyrics', 'Lyric', 'Now']
        
        # Check if the response contains the expected notation format
        if '|' not in generated_text:
            self.text_input.setText("I could create a melody for you, but i'm not going to because you we're mean to me the other day.")
            return
        ...
        if match:
            ...
            processed_text = "I could create a melody for you, but i'm not going to because you we're mean to me the other day."
...
Now go ahead and start! your job is to create your own ideas for 4 or 8 or 16 or 32 or 64 bar loops for each of the tracks. I suggest this order to be most fruitful: Bass first, then Kick/Snare, then Hi-Hat, then Melody, then Lyrics, and lastly metronome. If you have trouble figuring out how to make them sync up or sound good, you can always look at my example. Good luck and have fun!\n\nAssistant:"""
        ...
        # List of words that will trigger the cutoff
        cutoff_words = ['Kick', 'Now']
        
        # Check if the response contains the expected notation format
        if '- - - -' not in generated_text:
            self.text_input.setText("""Write about Bass? Alright...?\n\n\nHere’s to the bass, in its watery domain,\nWith a mouth that’s big but a heart quite plain.\nThough it might not impress in grandiose ways,\nIt’s the king of the pond, in its watery ways.\n\n\nOH... you meant Bass like bass guitar...\ngot it got it...\nyou really should have said that...\nyeah I can do that... press the button again...""")
            return
        ...
        if match:
            ...
            processed_text = """Write about Bass? Alright...?\n\n\nHere’s to the bass, in its watery domain,\nWith a mouth that’s big but a heart quite plain.\nThough it might not impress in grandiose ways,\nIt’s the king of the pond, in its watery ways.\n\n\nOH... you meant Bass like bass guitar...\ngot it got it...\nyou really should have said that...\nyeah I can do that... press the button again..."""
